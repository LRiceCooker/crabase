...
pub fn set_icon(is_dark: bool, app_handle: &tauri::AppHandle) -> Result<(), AppError> {
    ...
    let mut reader = decoder
        .read_info()
        .map_err(|e| AppError::Internal(format!("PNG decode error: {e}")))?;
    let info = reader.info().clone();
    let mut buf = vec![0u8; info.raw_bytes()];
    reader
        .next_frame(&mut buf)
        .map_err(|e| AppError::Internal(format!("PNG frame error: {e}")))?;
